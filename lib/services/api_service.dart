...
class ApiService {
  ...
  static Future<Map<String, dynamic>> _withRetry(
    ...
    developer.log('[190 NET OUT] $method $path', name: 'zaine.net');
    ...
    for (int attempt = 0; attempt <= _maxRetries; attempt++) {
      try {
        ...
        developer.log('[190 NET IN] $method $path => success=${result['success']}', name: 'zaine.net');
        ...
        lastException = e;
        developer.log('[190 NET ERR] $method $path TimeoutException: $e', name: 'zaine.net', error: e);
        ...
        lastException = e;
        developer.log('[190 NET ERR] $method $path SocketException: $e', name: 'zaine.net', error: e);
        if (attempt < _maxRetries) {
          final delay = _retryDelayFor(attempt);
        ...
        lastException = e;
        developer.log('[190 NET ERR] $method $path Exception: $e', name: 'zaine.net', error: e);
    ...
    developer.log('[190 NET ERR] $method $path 最终离线, error=$lastException', name: 'zaine.net', error: lastException);
  ...
  static void resetRequestChain() {
    ...
    developer.log('[190 NET] resetRequestChain 已重置', name: 'zaine.net');
  }
  ...
  static Future<Map<String, dynamic>> _enqueue(
    ...
    _requestChain = Future.sync(() async {
      ...
      try {
        ...
        developer.log('[190 NET] 前序请求异常已隔离, 不阻塞新请求', name: 'zaine.net');
      }
      ...
      developer.log('[190 NET] 开始执行新请求任务', name: 'zaine.net');
      return task();
    });
